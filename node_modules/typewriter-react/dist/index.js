...
function TypeWriter(props) {
  var _props$input = props.input,
      ...
      cursor = _props$cursor === void 0 ? false : _props$cursor,
  ...
  function printStr(input, index, output) {
    setTimeout(function () {
      ...
      if (typeof input[index] === 'object') {
        if (input[index]['delete']) {
          if (input[index]['delete'] >= 1) {
            ...
            printStr(input, index, newOutput);
          } else {
            printStr(input, index + 1, output);
          }
        } else {
          if (input[index]['pause'] >= 1) {
            input[index]['pause'] = input[index]['pause'] - 1;
            printStr(input, index, output);
          } else {
            printStr(input, index + 1, output);
          }
        }
      ...
      if (typeof input[index] === 'string') {
        var _newOutput = output + input[index].charAt(0);

        setOutput(_newOutput);

        if (input[index].length === 1) {
          printStr(input, index + 1, _newOutput);
        } else {
          input[index] = input[index].substring(1, input[index].length);
          printStr(input, index, _newOutput);
        }
      }
    }, speed);
  }
  ...
  }, typed, cursor ? /*#__PURE__*/React__default.createElement("span", {
    className: "cursor",
    ref: cursorRef
  }, cursorChar) : null);
}
